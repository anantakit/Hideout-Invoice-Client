...
import { formatTHB, todayISO } from '../lib/utils'
...
export default function CreateInvoice() {
  ...
  const handleCustomerCreated = useCallback(
    ...
    [queryClient, handleCustomerSelect]
  )
    ...
    <>
      <div className="p-8 max-w-5xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">กรอกรายละเอียดด้านล่างเพื่อออกใบเสร็จรับเงิน</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <div className="space-y-6">
            ...
            <div className="card p-6">
              <h2 className="text-sm font-semibold text-gray-900 mb-4">รายละเอียดใบเสร็จ</h2>
              <div>
            ...
            <div className="card p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-gray-900">ผู้ชำระเงิน</h2>
                <button
                  type="button"
              ...
              <Controller
                ...
                render={({ field }) => (
                  <select
                    className={errors.customer_id ? 'input-error' : 'input'}
                    value={field.value}
                    onChange={(e) => {
                      field.onChange(e)
                      const cust = customersData?.data.find((c) => c.id === e.target.value)
            ...
            {/* ── Hotel Info ────────────────────────────────── */}
            <div className="card p-6">
              <h2 className="text-sm font-semibold text-gray-900 mb-4">รายละเอียดการเข้าพัก (ไม่บังคับ)</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="date" className="input" {...register('check_in_date')} />
                </div>
                <div>
                  <label className="label">วันที่ออก</label>
                  <input type="date" className="input" {...register('check_out_date')} />
                </div>
                <div>
                  ...
                  <input
                    ...
                    {...register('nights')}
                  />
                  <p className="text-xs text-gray-400 mt-1">หากระบุวันเข้า-ออก ระบบจะคำนวณให้อัตโนมัติ</p>
                </div>
              </div>
            </div>
            ...
            {/* ── Line Items ───────────────────────────────── */}
            <div className="card p-6">
              <h2 className="text-sm font-semibold text-gray-900 mb-4">รายการสินค้า</h2>
                    ...
                    >
                      ...
                      <div>
                        ...
                        {errors.items?.[index]?.quantity && (
                          <p className="field-error">{errors.items[index]!.quantity!.message}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {errors.items?.[index]?.unit_price && (
                          <p className="field-error">{errors.items[index]!.unit_price!.message}</p>
                        )}
                      </div>
            ...
            {/* ── Summary ──────────────────────────────────── */}
            <div className="card p-6">
              <h2 className="text-sm font-semibold text-gray-900 mb-4">สรุปยอด</h2>
              <div className="max-w-xs ml-auto">
                <div className="flex justify-between">
                  <span className="font-bold text-gray-900">ยอดชำระทั้งหมด</span>
                  <span className="font-bold text-lg text-brand-700">{formatTHB(total)}</span>
                </div>
              </div>
            </div>
